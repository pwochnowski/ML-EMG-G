"""MAT file loader for EMG data.

Loads EMG data from MATLAB .mat files, supporting the NinaPro database
format with continuous EMG streams and per-sample labels.
"""
...
class MatFileLoader(DataLoader):
    """Load EMG data from MATLAB .mat files (NinaPro format).
    
    Handles the NinaPro database format where each .mat file contains:
    - Continuous EMG stream with per-sample labels
    - Multiple variables including emg, stimulus, restimulus, repetition
    
    This loader returns EMGData objects representing continuous recordings
    with per-sample labels stored in metadata, suitable for windowing.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            sampling_rate: Sampling rate in Hz (NinaPro DB1 = 100Hz).
            file_pattern: Glob pattern for finding .mat files.
            label_mapping: Optional mapping for label remapping.
            ...
            emg_columns: Tuple of (start, end) column indices for EMG channels.
                        None = use all channels, (0, 8) = forearm only,
                        (0, 10) = all channels including finger extensors.
    ...
    def load_file(self, path: Path) -> EMGData:
        ...
        # (This is mainly for API compatibility; windowing uses sample_labels)
    ...
    def _extract_metadata(self, path: Path, mat_data: dict) -> dict:
        ...
        if "exercise" in mat_data:
            ...
            metadata["position"] = f"E{exercise}"  # Use exercise as position
        else:
